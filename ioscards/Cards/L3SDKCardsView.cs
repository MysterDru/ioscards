...
namespace ioscards
{
    class L3SDKCardOptions
    {
        public bool IsSwipable { get; set; }
    }

    public interface L3SDKCardsViewDelegate
    {
	...
	partial class L3SDKCardsView : UIView
	{
        ...
        public L3SDKCardsViewDelegate TheDelegate {get; set;}
		...
		public L3SDKCardsView (IntPtr handle) : base (handle)
		{
            this.Setup();
		}

        public L3SDKCardsView(CGRect frame) : base (frame)
        {
            this.Setup();
        ...
        {
            this.AddCardWithOptions(card, new L3SDKCardOptions());
        }

        public void AddCardWithOptions(UIView card, L3SDKCardOptions options)
        {
            this.Cards.Add(card);
        ...
        void SetupHeight()
        {
            ...
            if (this.Frame.Location.Y < 0)
            {
                ...
                {
                    this.TheDelegate.L3SDKCardsView_Scrolling(UISwipeGestureRecognizerDirection.Down);
                }
...
//            base.TouchesMoved(touches, evt);
            ...
            if (canScroll)
            {
                if (this.TheDelegate != null)
                {
                    this.TheDelegate.L3SDKCardsView_Scrolling(this.GestureDirection);
                }
            }
        ...
        public override void TouchesEnded(NSSet touches, UIEvent theEvent)
        {
            if (!this.GestureView.IsEqual(this))
            {
                ...
                if (Math.Abs(x) > this.Frame.Size.Width / 2)
                {
                    if (this.TheDelegate != null)
                    {
                        this.TheDelegate.L3SDKCardsView_CardWillRemove(this.GestureView);
                    }

                    this.RemoveView(this.GestureView);
                    if (this.TheDelegate != null)
                    {
                        this.TheDelegate.L3SDKCardsView_CardDidlRemove(this.GestureView);
                    }
                    ...
                    if (this.Cards.Count == 0)
                    {
                        if (this.TheDelegate != null)
                        {
                            this.TheDelegate.L3SDKCardsView_AllCardRemoved();
                        }
        ...
        bool CanScroll(UISwipeGestureRecognizerDirection scrollDirection)
        {
            if (scrollDirection == UISwipeGestureRecognizerDirection.Up && this.Frame.Location.Y < 0)
            {
                if (Math.Abs(this.Frame.Location.Y) >= Math.Abs(this.Frame.Size.Height - this.SuperviewFrame.Size.Height))
                {
                    if (this.TheDelegate != null)
                    {
                        this.TheDelegate.L3SDKCardsView_UpperLimitReached();
                    }
            ...
            else if (scrollDirection == UISwipeGestureRecognizerDirection.Down && this.Frame.Location.Y > 0)
            {
                if (Math.Abs(this.Frame.Location.Y) >= this.ZeroFrame.Location.Y)
                {
                    if (this.TheDelegate != null)
                    {
                        this.TheDelegate.L3SDKCardsView_BottomLimitReached();
                    }
